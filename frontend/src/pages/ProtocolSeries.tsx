...
export function ProtocolSeriesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setSigned(s)
    setQuick(q)
    setProjectName(s[0]?.projectName || q[0]?.projectName || 'Baustelle')
  }, [projectId])
  ...
  return (
    <div className="overflow-x-hidden">
      <PageTitle title={projectName} subtitle="Begehungen & Schnellnotizen" />
      ...
      {!signed.length && !quick.length ? (
        <p className="text-center text-sm text-zinc-500">Keine Protokolle für diese Baustelle.</p>
      ) : null}
