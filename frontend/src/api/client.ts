...
export async function listTaskPhotos(taskId: string): Promise<ReportPhotosResponse> {
  return api<ReportPhotosResponse>(`/api/tasks/${encodeURIComponent(taskId)}/photos`)
}
...
export async function uploadTaskPhoto(
  ...
  const url = resolveApiUrl(`/api/tasks/${encodeURIComponent(taskId)}/photos`)
  const headers: Record<string, string> = {}
  const token = getToken()
  if (token) headers['Authorization'] = `Bearer ${token}`
...
export async function deleteTaskPhoto(
  ...
  return api<{ ok: boolean; count: number; maxPhotos: number }>(
    `/api/tasks/${encodeURIComponent(taskId)}/photos/${encodeURIComponent(photoId)}`,
    { method: 'DELETE' },
  )
}
...
export async function uploadTaskSignature(
  taskId: string,
  file: File,
  signedByLabel?: string,
): Promise<{ ok: boolean; signature: ReportSignature | null }> {
  const fd = new FormData()
  fd.append('file', file, file.name || 'signature.png')
  if (signedByLabel?.trim()) {
    fd.append('signedByLabel', signedByLabel.trim())
  ...
  const url = resolveApiUrl(`/api/tasks/${encodeURIComponent(taskId)}/signature`)
  const headers: Record<string, string> = {}
  const token = getToken()
  if (token) headers['Authorization'] = `Bearer ${token}`
  ...
  return res.json() as Promise<{ ok: boolean; signature: ReportSignature | null }>
}

export async function deleteTaskSignature(taskId: string): Promise<{ ok: boolean; signature: null }> {
  return api<{ ok: boolean; signature: null }>(`/api/tasks/${encodeURIComponent(taskId)}/signature`, {
    method: 'DELETE',
  })
}
