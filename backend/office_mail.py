...
MSG_SENT_TASK_COMPLETION = "Aufgabenabschluss wurde ans Büro gesendet."
...
def send_task_completion_to_office(
    task: dict[str, Any],
    profile: dict[str, Any],
    to_email: str,
    *,
    ...
    site = str(task.get("projectName") or "—").strip() or "—"
    day = _format_date_de(task.get("dueDate"))
    subject = f"Aufgabenabschluss: {site} vom {day}"
    try:
        blob = build_task_completion_pdf_bytes(task, profile, resolve_logo=resolve_logo)
        ascii_fn, _desc = build_task_completion_attachment_names(task)
    except Exception:
        logger.exception("Aufgabenabschluss-Anhang für Mail konnte nicht erzeugt werden")
        return False, False, "Der Aufgabenabschluss konnte nicht erzeugt werden."

    company = _company_signoff_name(profile)
    body = (
        "Hallo,\n\n"
        f"anbei der Aufgabenabschluss zur Baustelle {site} vom {day}.\n\n"
        "Mit freundlichen Grüßen\n"
        f"{company}\n\n\n"
        "Powered by Freiraum Beratung"
    )
