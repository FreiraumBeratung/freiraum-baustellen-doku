"""Welle 20: Putz & Stuck — 300 Basisszenarien × 6 = 1800 Smoke-Fälle.

Mengen in Summary/Tätigkeiten, keine Putzschicht-Dubletten in Materialien,
Material-/Werkzeugvorschläge, POB, kurz/lang, Whisper, Dialekt.
"""
...
from putz_stuck_wave20_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_putz_stuck_wave20_")))
...
def _build_cases() -> list[Case]:
    ...
    for spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"PutzStuck20_{idx:03d}_{tag}",
                    variant=tag,
                    raw=fn(spec["raw"]),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="ps-wave20",
        projectName="Putz Stuck Welle 20",
...
def main() -> int:
    ...
    if failures:
        print("VIRTUAL-SPEECH-PUTZ-STUCK-WAVE20-SMOKE: FEHLER")
    ...
    print(f"VIRTUAL-SPEECH-PUTZ-STUCK-WAVE20-SMOKE: OK ({len(cases)}/{len(cases)})")
    return 0
