"""Welle 33: PANZEK-Pilot GaLaBau + Tiefbau + Straßenbau — 300 × 6 = 1800.

Welle-33-Variante: gleicher Kern wie W31, andere Szenarien und Reihenfolgen.
"""
...
from pilot_gala_tief_strasse_wave33_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_pilot_wave33_")))
...
def _build_cases() -> list[Case]:
    ...
    for trade, spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"Pilot33_{trade}_{idx:03d}_{tag}",
                    trade=trade,
                    raw=fn(spec["raw"]),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="pilot-wave33",
        projectName=f"PANZEK W33 {case.trade}",
        customerName="Lehnen",
        date="2026-06-17",
...
def main() -> int:
    cases = _build_cases()
    return run_smoke(cases, "VIRTUAL-SPEECH-PILOT-GALA-TIEF-STRASSE-WAVE33-SMOKE")
