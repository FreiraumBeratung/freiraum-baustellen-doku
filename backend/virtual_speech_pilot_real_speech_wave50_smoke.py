"""Welle 50: Pilot Real-Speech — 50 Basisszenarien × 6 = 300 Smoke-Fälle.

Echte Diktat-Muster (GaLaBau Langketten, Material, Morgen/Offen, Regen/Problem, Whisper).
Rein additiv zu bestehenden Wellen.
"""
...
from pilot_real_speech_wave50_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_pilot_real_speech_wave50_")))
...
def _token_match(token: str, text: str) -> bool:
    ...
    key = token.casefold()
    if key == "regen":
        return "regen" in low or "regnen" in low
    if key == "pflaster":
...
def _build_cases() -> list[Case]:
    ...
    for spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"PilotReal50_{idx:03d}_{tag}",
                    variant=tag,
                    raw=fn(spec["raw"]),
...
def main() -> int:
    ...
    if failures:
        print("VIRTUAL-SPEECH-PILOT-REAL-SPEECH-WAVE50-SMOKE: FEHLER")
    ...
    print(f"VIRTUAL-SPEECH-PILOT-REAL-SPEECH-WAVE50-SMOKE: OK ({len(cases)}/{len(cases)})")
    return 0
