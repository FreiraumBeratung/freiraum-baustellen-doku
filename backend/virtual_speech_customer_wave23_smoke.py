"""Welle 23: Kundengespräch-Vollständigkeit + Problem/Offen — Pilot-Smoke.

25 Basisszenarien × 7 Gewerke × 6 Varianten = 1.050 Fälle.
Fokus P2: reiche Kundentails (gelobt, weiterempfehlen, weitere Aufträge) nicht abgeschnitten.
Rein additiv.
"""
...
from customer_wave23_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_customer_wave23_")))
...
def _token_in_text(text: str, token: str) -> bool:
    ...
    if tok in low:
        return True
    if tok == "gesprochen" and ("gred" in low or "unterhalten" in low or "kundengespräch" in low or "kundengespraech" in low):
        return True
    if tok == "einverstanden" and "einverstandn" in low:
        return True
    if tok == "abgestimmt" and "abgred" in low:
        return True
    if tok == "gelobt" and ("lobt" in low or "gelob" in low or "zufrieden" in low):
        return True
    if tok == "lobt" and ("lobt" in low or "gelobt" in low or "zufrieden" in low):
        return True
    if tok == "weiterempfehl" and ("empfiehlt" in low or "empfehl" in low):
        return True
    if tok == "auftrag" and ("auftraege" in low or "aufträge" in low or "phase" in low):
        return True
    if tok == "zufrieden" and ("gelobt" in low or "lobt" in low or "einverstanden" in low or "happy" in low):
        return True
    if tok == "kundengespräch" and "kundengespraech" in low:
        return True
    return False
...
def _raw_has_rich_customer_markers(raw: str) -> bool:
    low = raw.casefold()
    return any(
        m in low
        for m in (
...
def _whisper_hard(text: str) -> str:
    ...
    extra = (
        ...
        (r"\bweiterempfehlen\b", "weiter empfehlen"),
        (r"\bä", "ae"),
        (r"\bö", "oe"),
        (r"\bü", "ue"),
        (r"\bß", "ss"),
...
def _broken_de(text: str) -> str:
    ...
    for a, b in (
        ...
        ("weiterempfehlen", "weiterempfehln"),
    ):
        out = out.replace(a, b)
    return out
...
def _scenario_to_case(trade: str, idx: int, tag: str, raw: str, spec: dict) -> Case:
    ...
    return Case(
        ...
        expect_customer_rich=bool(spec.get("cust_rich")),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="customer-wave23",
        projectName=f"Customer Wave23 {case.trade}",
...
def _check_p2(case: Case, customer: str, summary: str, failures: list[str]) -> None:
    ...
    for token in case.customer_must_not_contain:
        if case.variant in {"N", "B", "D"} and _contains_in_text(customer, token):
    ...
    if case.expect_customer_rich and case.variant in {"N", "B"}:
        low = customer.casefold()
        if len(customer.strip()) < 45:
            failures.append(
                f"{case.name}: [P2] customerTalk zu kurz/abgeschnitten (len={len(customer)}, got={customer!r})"
            )
        if _raw_has_rich_customer_markers(case.raw):
            if "weiterempfehl" in case.raw.casefold() and not (
                "weiterempfehl" in low or "empfiehlt" in low or "empfehl" in low
            ):
                failures.append(
                    f"{case.name}: [P2] Weiterempfehlung fehlt (got={customer!r})"
                )
            if ("gelobt" in case.raw.casefold() or "lobt" in case.raw.casefold()) and not (
                "gelobt" in low or "lobt" in low or "zufrieden" in low
            ):
                failures.append(
                    f"{case.name}: [P2] Lob/Zufriedenheit fehlt (got={customer!r})"
                )
            if "weitere auftrag" in case.raw.casefold() and "auftrag" not in low:
                failures.append(
                    f"{case.name}: [P2] weitere Aufträge fehlen (got={customer!r})"
                )
        thin_only = re.match(
            r"^(mit der kundin gesprochen|mit dem kunden gesprochen)\.?$",
            low,
        )
        if thin_only:
            failures.append(
                f"{case.name}: [P2] customerTalk nur Dünnsatz (got={customer!r})"
            )
...
def main() -> int:
    ...
    if failures:
        print("VIRTUAL-SPEECH-CUSTOMER-WAVE23-SMOKE: FEHLER")
    ...
    print("VIRTUAL-SPEECH-CUSTOMER-WAVE23-SMOKE: OK")
    print(f"Total cases: {len(cases)}")
    print("Trades: GaLaBau, Trockenbau, Fliesen, SHK, Hochbau, Tiefbau, Putz")
    print("Basisszenarien pro Gewerk: 25 (Kundengespräch-Vollständigkeit)")
    return 0
