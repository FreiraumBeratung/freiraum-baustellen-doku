"""Welle 31: PANZEK-Pilot GaLaBau + Tiefbau + Straßenbau — 300 × 6 = 1800.
...
from pilot_gala_tief_strasse_wave31_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_pilot_wave31_")))
...
def _build_cases() -> list[Case]:
    ...
    for trade, spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"Pilot31_{trade}_{idx:03d}_{tag}",
                    trade=trade,
                    raw=fn(spec["raw"]),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="pilot-wave31",
        projectName=f"PANZEK W31 {case.trade}",
        customerName="Lehnen",
        date="2026-06-17",
...
def main() -> int:
    cases = _build_cases()
    return run_smoke(cases, "VIRTUAL-SPEECH-PILOT-GALA-TIEF-STRASSE-WAVE31-SMOKE")
