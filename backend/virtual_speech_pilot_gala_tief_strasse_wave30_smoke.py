"""Welle 30: Pilot-Monster GaLaBau + Tiefbau + Straßenbau — 300 × 6 Varianten = 1800.

Komplett neue Fälle gegenüber Welle 29, gleiche Sphäre/Struktur. Rein additiv.
"""
...
from pilot_gala_tief_strasse_wave30_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_pilot_wave30_")))
...
def _whisper_light(text: str) -> str:
    ...
    pairs = (
        ...
        (r"\bTiefbord\b", "tief bord"),
        (r"\bRinnensteine\b", "rinnen steine"),
        (r"\bMuldensteine\b", "mulden steine"),
        (r"\bKG-Rohre\b", "ka ga rohre"),
        (r"\bHT-Rohr\b", "ht rohr"),
        (r"\bRasenkantensteine\b", "rasen kanten steine"),
        (r"\bBordsteine\b", "bord steine"),
        (r"\bNatursteinpflaster\b", "natur stein pflaster"),
        (r"\bGehwegpflaster\b", "geh weg pflaster"),
        (r"\bWPC-Terrasse\b", "wpc ter ras se"),
        (r"\bFiltervlies\b", "filter vlies"),
        (r"\bFeinsteinzeugplatten\b", "fein stein zeug platten"),
        (r"\bSichtschutzzaun\b", "sicht schutz zaun"),
    )
...
def _whisper_hard(text: str) -> str:
    ...
    extra = (
        ...
        (r"\beingearbeitet\b", "ein ge arbeitet"),
        (r"\bä", "ae"),
        (r"\bö", "oe"),
        (r"\bü", "ue"),
        (r"\bß", "ss"),
...
def _build_cases() -> list[Case]:
    ...
    for trade, spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"Pilot30_{trade}_{idx:03d}_{tag}",
                    trade=trade,
                    raw=fn(spec["raw"]),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="pilot-wave30",
        projectName=f"Pilot W30 {case.trade}",
...
def main() -> int:
    cases = _build_cases()
    return run_smoke(cases, "VIRTUAL-SPEECH-PILOT-GALA-TIEF-STRASSE-WAVE30-SMOKE")
