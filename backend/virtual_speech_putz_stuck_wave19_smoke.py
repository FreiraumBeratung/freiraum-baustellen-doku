"""Welle 19: Putz & Stuck — 150 Basisszenarien × 6 = 900 Smoke-Fälle.

Material-/Werkzeugvorschläge, POB-Struktur, kurz/lang, Whisper, Dialekt.
"""
...
from putz_stuck_wave19_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_putz_stuck_wave19_")))
...
def _build_cases() -> list[Case]:
    ...
    for spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"PutzStuck19_{idx:03d}_{tag}",
                    variant=tag,
                    raw=fn(spec["raw"]),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="ps-wave19",
        projectName="Putz Stuck Welle 19",
...
def main() -> int:
    ...
    for case in cases:
        ...
        elif case.expect_customer and case.variant in {"N", "B", "M"}:
    ...
    if failures:
        print("VIRTUAL-SPEECH-PUTZ-STUCK-WAVE19-SMOKE: FEHLER")
    ...
    print(f"VIRTUAL-SPEECH-PUTZ-STUCK-WAVE19-SMOKE: OK ({len(cases)}/{len(cases)})")
    return 0
