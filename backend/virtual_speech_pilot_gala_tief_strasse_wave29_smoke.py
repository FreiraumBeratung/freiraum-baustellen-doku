"""Welle 29: Pilot-Monster GaLaBau + Tiefbau + Straßenbau — 300 × 6 Varianten = 1800.

Fokus Pilotkunde: GaLaBau mit Straßenarbeiten. Straßenbau über Tiefbau/GaLaBau-Engine.
Rein additiv.
"""
...
from pilot_gala_tief_strasse_wave29_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_pilot_wave29_")))
...
def _whisper_light(text: str) -> str:
    ...
    pairs = (
        ...
        (r"\bTerrassenplatten\b", "terras sen platten"),
    )
...
def _build_cases() -> list[Case]:
    ...
    for trade, spec in all_base_scenarios():
        ...
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"Pilot29_{trade}_{idx:03d}_{tag}",
                    trade=trade,
                    raw=fn(spec["raw"]),
...
def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="pilot-wave29",
        projectName=f"Pilot W29 {case.trade}",
...
def main() -> int:
    cases = _build_cases()
    return run_smoke(cases, "VIRTUAL-SPEECH-PILOT-GALA-TIEF-STRASSE-WAVE29-SMOKE")
