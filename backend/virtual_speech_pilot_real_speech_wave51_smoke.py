"""Welle 51: Pilot Real-Speech — 40 Basisszenarien × 6 = 240 Smoke-Fälle.

Diktat-Muster Session 11.07.: gebrochenes Deutsch, Langketten Unterbau/Pflaster/Fugen,
Wetter-Abbruch, Material/Kubik. Rein additiv zu bestehenden Wellen.
"""

from __future__ import annotations

import os
import re
import sys
import tempfile
import uuid
from dataclasses import dataclass
from pathlib import Path
from typing import Callable

BACKEND_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(BACKEND_DIR))

from main import StructureReportBody, api_structure_report  # noqa: E402
from app.services.problem_open_builder import _is_work_polluted as _problem_open_work_polluted  # noqa: E402
from app.services.summary_material_guard import summary_has_material_echo  # noqa: E402
from app.services.tenant_storage import TenantStore  # noqa: E402
from pilot_real_speech_wave51_scenarios import all_base_scenarios  # noqa: E402
from smoke_isolation import isolate_smoke_data  # noqa: E402

isolate_smoke_data(Path(tempfile.mkdtemp(prefix="freiraum_pilot_real_speech_wave51_")))
_STORE = TenantStore(str(uuid.uuid4()))


@dataclass(frozen=True)
class Case:
    name: str
    variant: str
    raw: str
    strict: bool
    expect_activities: tuple[str, ...]
    act_tokens: tuple[str, ...]
    expect_materials: tuple[str, ...]
    mat_tokens: tuple[str, ...]
    expect_machine_hours: tuple[str, ...]
    expect_machine_suggestions: tuple[str, ...]
    expect_suggestions: tuple[str, ...]
    forbid_activities: tuple[str, ...]
    forbid_materials: tuple[str, ...]
    forbid_suggestions: tuple[str, ...]
    expect_problem: bool
    expect_open: bool
    expect_customer: bool
    min_activity_count: int
    prob_must: tuple[str, ...]
    prob_not: tuple[str, ...]
    open_must: tuple[str, ...]
    open_not: tuple[str, ...]
    cust_must: tuple[str, ...]
    mat_echo: bool
    sum_qty: tuple[str, ...]
    sum_not: tuple[str, ...]
    forbid_future_pflaster: bool


def _contains_any(haystack: list[str], needle: str, *, variant: str = "N") -> bool:
    n = needle.casefold()
    if any(n in str(item).casefold() for item in haystack):
        return True
    if variant in {"W", "H"}:
        tokens = [t for t in re.split(r"[\s\-/]+", n) if len(t) > 3]
        if tokens:
            for item in haystack:
                low = str(item).casefold()
                hits = sum(1 for t in tokens if t in low)
                if hits >= max(1, len(tokens) - 1):
                    return True
    return False


def _contains_in_text(haystack: str, needle: str) -> bool:
    return needle.casefold() in haystack.casefold()


def _any_contains_in_list(items: list[str], needle: str) -> bool:
    return any(_contains_in_text(x, needle) for x in items)


def _token_match(token: str, text: str) -> bool:
    low = text.casefold()
    key = token.casefold()
    if key == "regen":
        return "regen" in low or "regnen" in low or "geregnet" in low
    if key == "unterbrochen":
        return "unterbrochen" in low or "abbrechen" in low
    if key == "pflaster":
        return "pflaster" in low or "pflastern" in low
    if key == "verschoben":
        return "verschoben" in low or "verschiebt" in low
    if key == "fugensand":
        return "fugensand" in low or "fugen" in low
    if key == "fugen":
        return "fugen" in low or "fugensand" in low
    if key == "unterbau":
        return "unterbau" in low or "untergrund" in low
    if key == "frostschutz":
        return "frostschutz" in low or "frostschutzschotter" in low
    if key == "kundin":
        return "kund" in low
    if key == "bauherr":
        return "bauherr" in low
    return key in low


def _count_token_hits(items: list[str], tokens: tuple[str, ...]) -> int:
    if not tokens:
        return 0
    hits = 0
    for item in items:
        if any(_token_match(tok, str(item)) for tok in tokens):
            hits += 1
    return hits


def _has_customer_talk(text: str) -> bool:
    low = text.casefold()
    return any(
        h in low
        for h in (
            "kund",
            "bauherr",
            "bauleitung",
            "auftraggeber",
            "gesprochen",
            "gred",
            "informiert",
            "abgestimmt",
            "zufrieden",
            "weiterempfehl",
            "kundengespräch",
            "kundengespraech",
            "unterhalten",
            "einverstanden",
        )
    )


def _whisper_light(text: str) -> str:
    out = text
    pairs = (
        (r"\bm³\b", "kubik"),
        (r"\bm²\b", "quadratmeter"),
        (r"\bSchotter\b", "schotter"),
        (r"\bSplitt\b", "split"),
        (r"\bSplit\b", "split"),
        (r"\bPflaster\b", "pflaster"),
        (r"\bPflastern\b", "pflastern"),
        (r"\bUntergrund\b", "untergrund"),
        (r"\bUnterbau\b", "unterbau"),
        (r"\bGraben\b", "graben"),
        (r"\bBagger\b", "bagger"),
        (r"\bQuadratmeter\b", "quadrat meter"),
        (r"\bKubikmeter\b", "kubik meter"),
        (r"\bzwei Fünfer\b", "zwei fuenfer"),
        (r"\bdrei Kubik\b", "drei kubik"),
        (r"\bzwei Kubik\b", "zwei kubik"),
        (r"\bnull 32\b", "null 32"),
        (r"\b0/32\b", "null 32"),
        (r"\bFrostschutz\b", "frostschutz"),
        (r"\bFugensand\b", "fugensand"),
    )
    for pat, repl in pairs:
        out = re.sub(pat, repl, out, flags=re.IGNORECASE)
    return out.lower()


def _whisper_hard(text: str) -> str:
    out = _whisper_light(text)
    extra = (
        (r"\bgebaggert\b", "ge baggert"),
        (r"\bverdichtet\b", "ver dichtet"),
        (r"\beingebaut\b", "ein gebaut"),
        (r"\bverarbeitet\b", "ver arbeitet"),
        (r"\babbrechen\b", "ab brechen"),
        (r"\babrechnen\b", "ab rechnen"),
        (r"\bregnen\b", "regnen"),
        (r"\bä", "ae"),
        (r"\bö", "oe"),
        (r"\bü", "ue"),
        (r"\bß", "ss"),
    )
    for pat, repl in extra:
        out = re.sub(pat, repl, out, flags=re.IGNORECASE)
    return out


def _broken_de(text: str) -> str:
    out = text
    for a, b in (
        ("haben wir", "hamma"),
        ("Problem:", "problem is"),
        ("Offen:", "offen is"),
        ("abbrechen", "abrechnen"),
        ("gesprochen", "gred"),
    ):
        out = out.replace(a, b)
    return out


def _dialect_de(text: str) -> str:
    out = text
    out = re.sub(r"\bhaben wir\b", "hamma", out, flags=re.IGNORECASE)
    out = re.sub(r"\bgesprochen\b", "gred", out, flags=re.IGNORECASE)
    return out


def _mega_runon(text: str) -> str:
    core = re.sub(r"[.!?]+\s*", " und also ", text)
    return f"Ja also vom Tagher {core} und Feierabend."


def _build_cases() -> list[Case]:
    builders: list[tuple[str, Callable[[str], str]]] = [
        ("N", lambda t: t),
        ("W", _whisper_light),
        ("H", _whisper_hard),
        ("B", _broken_de),
        ("D", _dialect_de),
        ("M", _mega_runon),
    ]
    cases: list[Case] = []
    idx = 1
    for spec in all_base_scenarios():
        acts = tuple(spec.get("acts") or ())
        min_count = spec["min_act"] if spec.get("min_act") is not None else len(acts)
        for tag, fn in builders:
            cases.append(
                Case(
                    name=f"PilotReal51_{idx:03d}_{tag}",
                    variant=tag,
                    raw=fn(spec["raw"]),
                    strict=bool(spec.get("strict")),
                    expect_activities=acts,
                    act_tokens=tuple(spec.get("act_tokens") or ()),
                    expect_materials=tuple(spec.get("mats") or ()),
                    mat_tokens=tuple(spec.get("mat_tokens") or ()),
                    expect_machine_hours=tuple(spec.get("mach") or ()),
                    expect_machine_suggestions=tuple(spec.get("mach_sug") or ()),
                    expect_suggestions=tuple(spec.get("sugs") or ()),
                    forbid_activities=tuple(spec.get("forbid_acts") or ()),
                    forbid_materials=tuple(spec.get("forbid_mats") or ()),
                    forbid_suggestions=tuple(spec.get("forbid_sugs") or ()),
                    expect_problem=bool(spec.get("problem")),
                    expect_open=bool(spec.get("open_")),
                    expect_customer=bool(spec.get("customer")),
                    min_activity_count=min_count,
                    prob_must=tuple(spec.get("prob_must") or ()),
                    prob_not=tuple(spec.get("prob_not") or ()),
                    open_must=tuple(spec.get("open_must") or ()),
                    open_not=tuple(spec.get("open_not") or ()),
                    cust_must=tuple(spec.get("cust_must") or ()),
                    mat_echo=bool(spec.get("mat_echo")),
                    sum_qty=tuple(spec.get("sum_qty") or ()),
                    sum_not=tuple(spec.get("sum_not") or ()),
                    forbid_future_pflaster=bool(spec.get("forbid_future_pflaster")),
                )
            )
            idx += 1
    return cases


def _run_case(case: Case) -> dict:
    body = StructureReportBody(
        projectId="pilot-real-wave51",
        projectName="Schmitz Außenanlage",
        customerName="Testkunde",
        date="2026-07-11",
        employeeNames=["Max", "Dennis"],
        startTime="06:00",
        endTime="17:30",
        exportFormat="PDF",
        rawText=case.raw,
    )
    return (api_structure_report(body, store=_STORE).get("structured") or {})


def _check_pob(case: Case, probs: list[str], opens: list[str], failures: list[str]) -> None:
    if case.expect_problem:
        if not probs:
            failures.append(f"{case.name}: problems leer")
        elif case.variant in {"N", "B", "M"}:
            joined = " ".join(probs)
            for token in case.prob_must:
                if not (
                    _any_contains_in_list(probs, token)
                    or _contains_in_text(joined, token)
                    or any(_token_match(token, p) for p in probs)
                ):
                    failures.append(f"{case.name}: problem fehlt {token!r} (got={probs!r})")
            if case.variant == "N":
                for token in case.prob_not:
                    if _any_contains_in_list(probs, token):
                        failures.append(f"{case.name}: problem verboten {token!r} (got={probs!r})")
            for item in probs:
                if _problem_open_work_polluted(item) and not any(
                    _token_match(t, item) for t in case.prob_must if t in {"liefer", "spät", "spaet"}
                ):
                    failures.append(f"{case.name}: Arbeitstext in problem (got={item!r})")

    if case.expect_open:
        if not opens:
            failures.append(f"{case.name}: openItems leer")
        elif case.variant in {"N", "B", "M"}:
            joined = " ".join(opens)
            for token in case.open_must:
                if not (
                    _any_contains_in_list(opens, token)
                    or _contains_in_text(joined, token)
                    or any(_token_match(token, o) for o in opens)
                ):
                    failures.append(f"{case.name}: offen fehlt {token!r} (got={opens!r})")
            if case.variant == "N":
                for token in case.open_not:
                    if _any_contains_in_list(opens, token):
                        failures.append(f"{case.name}: offen verboten {token!r} (got={opens!r})")
            for item in opens:
                if _problem_open_work_polluted(item):
                    failures.append(f"{case.name}: Arbeitstext in openItems (got={item!r})")


def main() -> int:
    os.environ["OPENAI_API_KEY"] = ""
    cases = _build_cases()
    failures: list[str] = []

    for case in cases:
        structured = _run_case(case)
        acts = [str(x) for x in (structured.get("activities") or [])]
        mats = [str(x) for x in (structured.get("materials") or [])]
        sugs = [str(x) for x in (structured.get("materialSuggestions") or [])]
        mach_sugs = [str(x) for x in (structured.get("machineSuggestions") or [])]
        machine_hours = [str(x) for x in (structured.get("machineHours") or [])]
        probs = [str(x) for x in (structured.get("problems") or [])]
        opens = [str(x) for x in (structured.get("openItems") or [])]
        customer = str(structured.get("customerTalk") or "")
        summary = str(structured.get("summary") or "")

        if case.min_activity_count > 0 and not case.act_tokens and not case.strict:
            if len(acts) < case.min_activity_count and case.variant in {"N", "B"}:
                failures.append(
                    f"{case.name}: zu wenige Tätigkeiten ({len(acts)} < {case.min_activity_count}) got={acts!r}"
                )

        if len(acts) < case.min_activity_count and case.strict:
            skip_min = (
                (case.variant in {"W", "H"} and not acts)
                or (case.variant == "M" and len(acts) >= max(0, case.min_activity_count - 2))
            )
            if not skip_min:
                failures.append(
                    f"{case.name}: zu wenige Tätigkeiten ({len(acts)} < {case.min_activity_count}) got={acts!r}"
                )

        if case.expect_activities or case.act_tokens or case.expect_materials or case.mat_tokens:
            if case.strict and case.variant in {"N", "B"}:
                for expected in case.expect_activities:
                    if not _contains_any(acts, expected, variant=case.variant):
                        failures.append(f"{case.name}: activity fehlt -> {expected} (got={acts!r})")
                for expected in case.expect_materials:
                    if not _contains_any(mats, expected, variant=case.variant):
                        failures.append(f"{case.name}: material fehlt -> {expected} (got={mats!r})")
            elif case.variant in {"N", "B", "M"} and case.min_activity_count > 0 and case.act_tokens:
                if case.variant in {"W", "H"} and not acts:
                    pass
                else:
                    hits = _count_token_hits(acts, case.act_tokens)
                    if hits < 1:
                        failures.append(
                            f"{case.name}: act_tokens zu wenig Treffer ({hits}) "
                            f"tokens={case.act_tokens!r} got={acts!r}"
                        )
            if case.variant in {"N", "B"} and case.mat_tokens:
                for token in case.mat_tokens:
                    if not any(_token_match(token, m) for m in mats):
                        failures.append(f"{case.name}: mat_token fehlt -> {token} (got={mats!r})")

        for expected in case.expect_machine_hours:
            if not _contains_any(machine_hours, expected, variant=case.variant):
                failures.append(f"{case.name}: maschinenstunden fehlen -> {expected} (got={machine_hours!r})")

        if case.variant in {"N", "B"}:
            for expected in case.expect_machine_suggestions:
                if not _contains_any(mach_sugs, expected, variant=case.variant):
                    failures.append(f"{case.name}: maschinenvorschlag fehlt -> {expected} (got={mach_sugs!r})")
            for expected in case.expect_suggestions:
                if not _contains_any(sugs, expected, variant=case.variant):
                    failures.append(f"{case.name}: materialvorschlag fehlt -> {expected} (got={sugs!r})")

        for forbidden in case.forbid_activities:
            if _contains_any(acts, forbidden, variant=case.variant):
                failures.append(f"{case.name}: activity verboten -> {forbidden} (got={acts!r})")

        if case.variant in {"N", "B"}:
            for forbidden in case.forbid_materials:
                if _contains_any(mats, forbidden, variant=case.variant):
                    failures.append(f"{case.name}: material verboten -> {forbidden} (got={mats!r})")
            if case.forbid_future_pflaster:
                if any(
                    re.search(r"pflaster\s+verlegt|pflaster\s+gelegt", x, flags=re.IGNORECASE) for x in acts
                ):
                    failures.append(f"{case.name}: Zukunfts-Pflaster in activities (got={acts!r})")
                if case.variant == "N" and _contains_in_text(summary, "pflaster verlegt"):
                    failures.append(f"{case.name}: Zukunfts-Pflaster in summary (got={summary!r})")

        if acts and case.min_activity_count > 0 and (not summary or len(summary.strip()) < 8):
            failures.append(f"{case.name}: summary leer/zu kurz (got={summary!r})")

        if case.sum_qty and case.variant in {"N", "B", "M"}:
            for token in case.sum_qty:
                if not _contains_in_text(summary, token):
                    failures.append(f"{case.name}: summary Menge fehlt {token!r} (got={summary!r})")

        if case.sum_not and case.variant == "N":
            for token in case.sum_not:
                if _contains_in_text(summary, token):
                    failures.append(f"{case.name}: summary verboten {token!r} (got={summary!r})")

        if case.mat_echo and case.variant == "N" and mats and summary_has_material_echo(summary, mats, acts):
            failures.append(f"{case.name}: Material-Echo in summary (got={summary!r})")

        _check_pob(case, probs, opens, failures)

        if case.expect_customer and case.variant in {"N", "B", "M"}:
            if not _has_customer_talk(customer):
                failures.append(f"{case.name}: customerTalk fehlt (got={customer!r})")
            else:
                for token in case.cust_must:
                    if token == "gesprochen" and _contains_in_text(customer, "gred"):
                        continue
                    if not _contains_in_text(customer, token):
                        failures.append(f"{case.name}: kunde fehlt {token!r} (got={customer!r})")

    if failures:
        print("VIRTUAL-SPEECH-PILOT-REAL-SPEECH-WAVE51-SMOKE: FEHLER")
        print(f"Total cases: {len(cases)}")
        for row in failures[:80]:
            print(" -", row)
        if len(failures) > 80:
            print(f" ... und {len(failures) - 80} weitere")
        return 1

    print(f"VIRTUAL-SPEECH-PILOT-REAL-SPEECH-WAVE51-SMOKE: OK ({len(cases)}/{len(cases)})")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
